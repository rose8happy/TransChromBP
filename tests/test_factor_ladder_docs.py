...
def test_tracking_registers_factor_ladder_as_live_row() -> None:
    ...
    assert "参数漂移" in factor_row["当前结论 / 进度"]
    assert "对齐 exporter CLI" in factor_row["下一步"]
    assert "最小 smoke" in factor_row["下一步"]
    assert "不要继续推进 E3" in factor_row["下一步"]


def test_tracking_registers_loss_balance_closeout_row() -> None:
    rows = parse_markdown_table("TRACKING.md")
    loss_row = next(
        row for row in rows if row["事项"] == "loss balance curriculum"
    )

    assert loss_row["当前状态"] == "待处理"
    assert "selector-only" in loss_row["当前结论 / 进度"]
    assert "dynamic-count seed42" in loss_row["当前结论 / 进度"]
    assert "clear gain" in loss_row["当前结论 / 进度"]
    assert "静态 safe-envelope" in loss_row["下一步"]
    assert "idle" in loss_row["下一步"]
...
def test_runs_manifest_records_factor_ladder_halt_and_loss_balance_closeout() -> None:
    ...
    assert (
        ...
        == "reports/analysis/loss_balance_selector_wave1_seed42_20260417.md"
    )
