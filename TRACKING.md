...
| 官方 ChromBPNet 外置化实施 | 进行中 | 截至 `2026-04-19`，`6000` 的真实 bridge smoke `chrombpnet_official_step3_bridge_smoke_20260408_110411` 仍是这条 infra 线的 terminal evidence，且 canonical closeout 已补到 `reports/chrombpnet_official_externalization_closeout_20260408.md`。当前可以确认的口径是：helper bridge 已在真机验证通过，唯一已验证的官方根路径就是 `/data1/zhoujiazhen/bylw_atac/chromBPNet`；implementation plan 与 patch ledger 现在只分别承担“怎么做”和“依赖哪些官方触点”，不再兼任结果报告。 | 若继续收这条线，下一步只剩清理少数历史文档里残留的 `chrombpnet_official` 旧写法，并判断 `autonomy/20260406-chrombpnet-externalization` 是否已满足卸载 mounted worktree 的条件。 | `reports/chrombpnet_official_externalization_closeout_20260408.md`、`reports/chrombpnet_official_patch_ledger_20260406.md`、`docs/plan/chrombpnet_official_externalization_design_20260406.md`、`docs/plan/chrombpnet_official_externalization_implementation_plan_20260406.md` |
...
| AI 指令体系 + Git 主档案清理 | 进行中 | 在先前 `master`/worktree 收口的基础上，本轮继续做了一次 broad audit：仍被 live 文档引用的 factor-ladder 设计/实现计划已迁回 `docs/plan/`；`docs/superpowers/` 现在只保留历史 agent 规划档案并补了目录说明；`reports/chrombpnet_official_externalization_closeout_20260408.md`、`reports/chrombpnet_official_patch_ledger_20260406.md` 与 `reports/foundation_cache_contract_snapshot_20260406.md` 已补齐外置化/infra branch 的 canonical 档案；过时的 `scripts/deploy_strict_compare_staging_to_6000.sh` 已退役；`a6000-dual-track-20260410` 与 `multiscale-decoder-probe-20260407` 两条 clean snapshot worktree 已卸载，registry 改为通过 snapshot tags 恢复。 | 下一步最值得做的是补齐 `dual-track-20260409` 与两条 `autonomy/*` 的剩余 closeout 证据链，再判断它们是否也可以像这轮一样继续收挂；目录 rename 仍应单开一轮，和绝对路径/链接修复一起做。 | `docs/env/repository_governance.md`、`reports/repository_governance_cleanup_20260419.md`、`reports/worktree_registry_audit_20260419.md`、`reports/project_broad_audit_cleanup_20260419.md`、`docs/plan/git_cleanup_and_archive_plan_20260321.md` |
