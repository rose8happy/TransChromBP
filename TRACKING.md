...
|---|---|---|---|---|
| 仓库任务盘点（2026-04-09） | 进行中 | 截至 `2026-04-09 23:16:50 CST`，本轮 worktree cleanup 已完成：`master` 已恢复为 clean 的 canonical 档案仓，并用本轮 docs closeout 收口主档案；实验/工程支线也已各自冻结为 clean worktree + 本地提交：`dual-track-20260409 -> 4ffc6e7 (wip: snapshot dual track pivot)`、`autonomy/20260406-structure -> 764b8c0`、`autonomy/20260406-chrombpnet-externalization -> 4aadffa`。当前 live 实验面按新解耦规则并列读取为：`6000` 目前 idle，teacher-distill 已停表，当前独立 backlog 首选 `AlphaGenome matched raw-track slice`；`6002` 仍在跑 `r4`，用于确认 `U-Net-lite v1` 的 rigor closeout。 | `6000` 只看自己的独立 backlog，优先推进 `AlphaGenome matched raw-track slice`；`6002` 先完成 `r4` 收口与 rigor 判读；若结果稳定 no-go，则把 `v1` 正式停表；若明显改善，唯一允许下一步是一次确认性 cheap rerun。 | `docs/plan/2026-04-09_dual_machine_experiment_charter.md`、`reports/ntv2_teacher_distill_short10_gate_closeout_20260409.md`、`reports/unet_lite_v1_rigor_review_20260409.md`、`reports/repository_status_handoff_20260409.md` |
...
| 6000 `NT v2 teacher-distill short10`（P0） | 进行中 | 截至 `2026-04-09 22:46 CST`，A6000 双卡对应的 `ntv2_teacher_distill_short10_s42_6000_20260409_r2` 已完成 formal gate closeout。使用 matched `short10_nofoundation_control` 的同口径 `valid peak` 做对照后，结论非常明确：control best `val:peak` 为 `0.336346 / 0.8006`，而 distill `r2` best `val:peak` 只有 `0.345324 / 0.5781`，既没有达到 `peak JSD` 改善 `>=0.002`，也没有满足 `count_r` 不下降超过 `0.005`；相反，`JSD` 变差约 `+0.009`，`count_r` 下降约 `-0.223`。因此这条线按原 stop-rule 正式判负：不升 `full-budget distill`，不扩 seed，tutorial teacher-distill 线当前停表。 | 不再给 tutorial `teacher-distill` 分配后续 `6000` run；`6000` 当前不再等待 `6002`，下一次 A6000 占卡默认给自己的独立高价值 backlog，首选 `AlphaGenome matched raw-track slice`；若该任务尚未 ready，则先完成其前置准备，不让 `6000` 空等。 | `docs/plan/2026-04-09_dual_machine_experiment_charter.md`、`reports/ntv2_teacher_distill_short10_gate_closeout_20260409.md`、`reports/repository_status_handoff_20260409.md` |
