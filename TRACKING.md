...
| U-Net vs AlphaGenome 架构复核（2026-04-11） | 进行中 | 截至 `2026-04-11 23:45:00 CST`，复核结论、正式 spec 与实现计划均已落盘；用户已确认进入执行阶段。执行约束已固定为：代码实现与 smoke / probe 以 `6000` `/data1/zhoujiazhen/bylw_atac/.codex_jobs/alphagenome_factor_ladder_20260411` 和 `6002` `/home/zhengwei/bylw_atac/.codex_jobs/alphagenome_factor_ladder_20260411` 这两个远端隔离工作区为主，本地仓仅保留 canonical spec / plan / report / tracking。`Task 1 / E1 long-context scaffold` 已收口：head=`231ed2c`，`pytest tests/test_long_context_probe_launcher.py -q` 为 `4 passed`，`bash -n` 与 1-step dry-run 已通过。`Task 2 / E2 hierarchical model variant` 已收口：head=`ffcc569`，`pytest tests/test_hierarchical_transchrombp.py -q` 为 `6 passed`，并已补齐两项关键契约：对 `foundation/genos/caduceus/genos_cached/profile_decoder` 等 unsupported section 显式报错，不再 silent no-op；`load_bias_branch_weights()` 已复用 baseline `_load_checkpoint()`，恢复与 baseline 一致的 bias checkpoint 兼容语义。`Task 3 / hierdec4096 probe wiring` 已收口：head=`c4795d0`，`pytest tests/test_factor_ladder_launchers.py -q` 为 `4 passed`，launcher 默认输出目录已切到隔离工作区 `outputs/`，并用 `bash -n + stubbed python argv capture` 锁住真实调用绑定。`Task 4 / model-teacher cache export` 已收口：head=`be0abc8`，`pytest tests/test_model_teacher_cache_export.py -q` 为 `4 passed`，导出端 `profile16` pooling 已改成直接复用训练侧 `_pool_profile_probs_to_bins`，并且构建导出 dataset 时会显式剥离 `teacher_cache_dir/teacher_target_names`，不再对旧 teacher cache 产生隐式依赖。`Task 5 / hierdec4096 distill wiring` 也已在同一 6000 隔离 git 工作区完成并通过 implementer + spec review + code quality review 三道 gate：当前 head=`6acb15f`，新增 `train_tutorial_teacher_v2_hierdec4096_distill_short10.yaml` 与 `run_teacher_v2_hierdec4096_distill.sh`，并把 `tests/test_factor_ladder_launchers.py` 扩到 `8 passed`；distill launcher 现在会在 `train+valid` 两个 manifest 都存在时跳过 exporter，且 skip-export 分支不再要求 `TEACHER_CKPT`。6002 镜像已同步。当前仍未启动正式 run。 | 进入 `Task 6`：把 `alphagenome_factor_ladder` 注册进 live docs / registry，并补 docs regression；之后再决定是否做一条真实的小规模 exporter+distill smoke。 | `docs/superpowers/specs/2026-04-11-alphagenome-like-factor-ladder-design.md`、`docs/superpowers/plans/2026-04-11-alphagenome-like-factor-ladder.md`、`reports/unet_vs_alphagenome_reassessment_20260411.md`、`/data1/zhoujiazhen/bylw_atac/.codex_jobs/alphagenome_factor_ladder_20260411`、`/home/zhengwei/bylw_atac/.codex_jobs/alphagenome_factor_ladder_20260411`、`reports/unet_lite_v1_rigor_review_20260409.md`、`reports/multiscale_local_skip_v2_30ep_gate_closeout_20260410.md`、`reports/alphagenome_matched_raw_track_slice_v2_closeout_20260410.md`、`/home/zhengwei/.config/superpowers/worktrees/chromBPNet/dual-track-20260409/vendor/transchrombp/transchrombp/models/profile_decoder.py`、`/home/zhengwei/.config/superpowers/worktrees/chromBPNet/dual-track-20260409/vendor/transchrombp/transchrombp/models/transchrombp.py` |
