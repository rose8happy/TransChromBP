...
|---|---|---|---|---|
| 仓库任务盘点（2026-04-09） | 进行中 | 截至 `2026-04-10 03:03:02 CST`，本轮 worktree cleanup 仍保持收口态：`master` 是 clean 的 canonical 档案仓，冻结支线继续保留；`6000` 双轨中的 A6000 formal gate `teacher_v2_center_pool_msdls_v2_30ep_s42_6000_20260410_r1` 仍在 active training，log 为 `/data1/zhoujiazhen/bylw_atac/TransChromBP/logs/teacher_v2_center_pool_msdls_v2_30ep_s42_6000_20260410_r1.log`，训练进程仍在跑，日志已推进到 `epoch=1 step=9200/9841`，且只和历史 `corrected B` 做 formal gate 比较。并行 sidecar `alphagenome_matched_raw_track_slice_v2_20260410` 已经完成，log 为 `/data1/zhoujiazhen/bylw_atac/TransChromBP/logs/alphagenome_matched_raw_track_slice_v2_20260410.log`；当前已确认产物 `summary.csv`、`merged_locus_totals.csv`、`region_metadata.jsonl`、`run_meta.json` 与完整 `profiles/*.npz` 全部落盘。两条线在 launch 之后互不等待；AlphaGenome 仍只算窄 external coordinate，不升级成大 benchmark。A6000 初始 ETA 窗口仍暂维持在 `2026-04-10 07:48:42 CST` 到 `2026-04-10 10:48:42 CST`，待首个完整 validation epoch 出现后再收紧。 | A6000 继续看实时日志：`tail -f /data1/zhoujiazhen/bylw_atac/TransChromBP/logs/teacher_v2_center_pool_msdls_v2_30ep_s42_6000_20260410_r1.log`；完成检查改为 `test -f /data1/zhoujiazhen/bylw_atac/TransChromBP/outputs/logs/teacher_v2_center_pool_msdls_v2_30ep_s42_6000_20260410_r1/run_meta.json && tail -n 20 /data1/zhoujiazhen/bylw_atac/TransChromBP/logs/teacher_v2_center_pool_msdls_v2_30ep_s42_6000_20260410_r1.log`。AlphaGenome 完成检查改为 `grep -E \"Wrote summary|Wrote merged comparison\" /data1/zhoujiazhen/bylw_atac/TransChromBP/logs/alphagenome_matched_raw_track_slice_v2_20260410.log && ls -l /data1/zhoujiazhen/bylw_atac/TransChromBP/outputs/alphagenome_pilot/alphagenome_matched_raw_track_slice_v2_20260410/{summary.csv,merged_locus_totals.csv}`。`6002` 继续按自己的 rigor closeout 文档收口。 | `docs/plan/2026-04-09_dual_machine_experiment_charter.md`、`reports/alphagenome_matched_raw_track_slice_v1_closeout_20260410.md`、`reports/unet_lite_v1_rigor_review_20260409.md`、`reports/repository_status_handoff_20260409.md` |
...
| 6002 RTX 3080 单卡实验侧线 | 进行中 | 截至 `2026-04-09 23:16:50 CST`，`6002` 的 3080 仍在跑 `teacher_v2_center_pool_unet_lite_v1_short10_s42_6002_20260409_r4`，目的不是扩线，而是验证 `r3` 的 `provisional no-go` 是否稳定。fresh evidence：GPU `100% util / 4287 MiB`；活跃训练进程已指向 `train_tutorial_teacher_v2_readout_short10_teacher_v2_center_pool_unet_lite_v1_short10_s42_6002_20260409_r4_runtime.yaml`、`data_tutorial_canonical_v1_6002.yaml` 与同一 `unet_lite_v1` model config；日志 `/home/zhengwei/bylw_atac/TransChromBP/logs/teacher_v2_center_pool_unet_lite_v1_short10_s42_6002_20260409_r4.log` 已越过 header，进入真实 training setup。关键配置差异相对 `r3` 只改了 `run_name` 与 `MASTER_PORT_BASE=30063`，其余 seed / batch / data config 保持同口径。按 `r3` 的真实节奏粗估，整条 `r4` 收口 ETA 窗口约为 `2026-04-10 00:50 CST` 到 `2026-04-10 01:30 CST`。 | 若 `r4` 仍落在 `r3` 附近，则 `U-Net-lite v1` 正式收成 no-go 并停表；若明显改善，则唯一允许下一步是一次确认性 cheap rerun。监控命令：`tail -f /home/zhengwei/bylw_atac/TransChromBP/logs/teacher_v2_center_pool_unet_lite_v1_short10_s42_6002_20260409_r4.log`。 | `docs/plan/2026-04-09_dual_machine_experiment_charter.md`、`reports/unet_lite_v1_rigor_review_20260409.md`、`reports/repository_status_handoff_20260409.md` |
...
| AlphaGenome 代码+权重 | `/data1/zhoujiazhen/bylw_atac/foundation_models/alphagenome/` | GitHub + HuggingFace / API | AlphaGenome 推理/对比 | 进行中（代码与 `all-folds` 目录已在；`2026-04-07` 已通过 `ALPHAGENOME_API_KEY` 认证与 `output_metadata()` 实机 smoke；`2026-04-10 00:22:55 CST` 已完成 `alphagenome_matched_raw_track_slice_v1_20260410` closeout，并以 technical/alignment gate `pass` 收口。下一步若继续，只能扩到约 `12-20 loci` 的稍大 matched panel，仍不升级成大 benchmark。） |
