...
> 当前存在 1 条 active 的 `6000` training run：`teacher_v2_center_pool_longctx4096_short10_s42_6000_20260411_r1`（`alphagenome_factor_ladder` / unattended matrix 当前 `E1 short10`，dual A6000，queue 接管时间 `2026-04-12 00:40:09 CST`，log 见 `docs/experiments/runs.csv`）；`teacher_v2_hierdec4096_short10_s42_6000_20260411_r1` 已于 `2026-04-11 23:06:08 CST` 正常完成，其余旧 `A6000 formal gate`、`AlphaGenome v2 sidecar` 与 `U-Net-lite v1` 都已收口。
...
|---|---|---|---|---|
| AlphaGenome-like factor ladder（E1/E2/E3） | 进行中 | `alphagenome_factor_ladder` 的 docs/spec/plan 已在 canonical repo 注册；`6000` remote isolated workdir 在 commit `5877077` 上完成的 single-GPU `exporter+distill` smoke `hierdec4096_exporter_distill_smoke_20260411_2110`（`32 train / 32 val`, `dry_run_steps=2`）已回填进 canonical archive base commit `079b603`，随后 unattended queue 启动层热修已归档到 canonical commit `ac55153`。正式 `E2` dual-A6000 gate `teacher_v2_hierdec4096_short10_s42_6000_20260411_r1` 已于 `2026-04-11 23:06:08 CST` 正常完成；其后 unattended matrix queue 在 `2026-04-12 00:40:09 CST` 成功接管，并启动 `E1` dual-A6000 run `teacher_v2_center_pool_longctx4096_short10_s42_6000_20260411_r1`。这轮 bring-up 的关键技术结论已经收口：`6000` 上 direct `torchrun --standalone` 会卡在 OMP warning / `c10d` socket timeout；queue 现已切到显式 `torchrun --nnodes=1 --master_addr=127.0.0.1 --master_port ...`，并保留流式日志、scrub 继承的 `MASTER_*`、且只用本次追加输出做 retry 判定。当前 queue state dir=`/data1/zhoujiazhen/bylw_atac/.codex_jobs/alphagenome_factor_ladder_20260411/outputs/queue/factor_ladder_unattended_20260411`，active stage=`e1_longctx4096_short10`；`E1` log=`/data1/zhoujiazhen/bylw_atac/.codex_jobs/alphagenome_factor_ladder_20260411/logs/teacher_v2_center_pool_longctx4096_short10_s42_6000_20260411_r1.log`，两张 A6000 当前约 `99%` 利用。按 `step 40/7380 @ 9.5s` 估算，`E1 short10` 当前 ETA 约 `2026-04-12 05:30-06:30 CST`。 | 让 unattended matrix queue 自然继续执行当前 `E1 -> E2 teacher30 -> export -> E3` 串行矩阵，只在技术熔断（OOM / NaN / DDP 失败 / checkpoint 或 manifest 缺失）时介入；`E1` 完成前，不手动并行再开 factor-ladder 新 run。当前队列里已登记的后续 run 是 `teacher_v2_hierdec4096_teacher30_s42_6000_20260411_r1` 与 `teacher_v2_hierdec4096_distill_short10_s42_6000_20260411_r1`。下一个最值得盯的检查点是 queue 是否在 `teacher_v2_center_pool_longctx4096_short10_s42_6000_20260411_r1` 结束后自动推进到 `teacher_v2_hierdec4096_teacher30_s42_6000_20260411_r1`。如需跟踪进度，直接看当前 run log、queue state dir 和 GPU。 | `docs/superpowers/specs/2026-04-11-alphagenome-like-factor-ladder-design.md`、`reports/unet_vs_alphagenome_reassessment_20260411.md`、`docs/experiments/registry.md` |
...
| AI 指令体系 + Git 主档案清理 | 进行中 | 在先前 `master`/worktree 收口的基础上，本轮继续把“多真源心智”压成显式 contract：已新增 `docs/env/repository_governance.md` 统一定义 local canonical trunk、6000 official lookup root、6000 runtime、6002 runtime mirror 四个角色；`scripts/sync_project.sh` 已改成 `publish-runtime-{6000,6002}`、`pull-results-{6000,6002}`、`status-all` 的显式接口，并让旧 `deploy` / `download_results` 直接报 deprecated；`README.md` / `DEVELOPMENT.md` / `AGENTS.md` 已同步到单向发布口径。根目录零散单文件也已进一步归位：`TRAINING_ANALYSIS.md`、`REPRODUCTION_NOTES.md`、`bpnet_model_annotated.py`、`visualize_bpnet.py` 已迁回 `reports/`、`docs/research/`、`docs/learning/assets/`、`scripts/`。 | 如果继续做整理，下一步最值得做的不是再扫一次同步脚本，而是对当前 mounted worktree 做 registry / closeout / tag 对账，决定哪些 family 已满足收口条件可以卸载；若仍想把本地仓物理目录从 `chromBPNet` 改成更直观的名字，也应单开一轮 rename，和绝对路径/链接修复一起做。 | `docs/env/repository_governance.md`、`reports/repository_governance_cleanup_20260419.md`、`docs/plan/git_cleanup_and_archive_plan_20260321.md`、`docs/plan/workspace_cleanup_plan_20260405.md` |
