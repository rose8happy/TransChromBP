...
> 当前仓库 / worktree / 双机实验的实时总览统一看 [reports/repository_status_handoff_20260409.md](reports/repository_status_handoff_20260409.md)；本节只保留结论级 live 状态。

| 事项 | 当前状态 | 当前结论 / 进度 | 下一步 | 关键路径 |
|---|---|---|---|---|
| 仓库任务盘点（2026-04-09） | 进行中 | 截至 `2026-04-09 21:30-21:31 CST` 重新盘点后，live 状态已不再是旧的 `msdec_v1_s2 + skipprobe_wide` 双机并跑，而是：本地 canonical git 档案仓仍是脏工作树的 `master`（ahead `20` commits；tracked 修改 `23`、untracked `31`），最新实验 pivot 实际落在 `dual-track-20260409` worktree；远端方面 `6000` 正在跑 `ntv2_teacher_distill_short10_s42_6000_20260409_r2`，`6002` 当前空闲，`U-Net-lite` 仍停留在“本地已实现、远端未起跑”。 | 先等 `6000` 的 `teacher-distill short10` 收口，再决定是否升 full 或改由 `6002` 起 `U-Net-lite`；与此同时继续按 `工作区整理 -> 论文收口 -> 小型工程尾项` 推进。 | `reports/repository_status_handoff_20260409.md`、`reports/project_plan_code_review_20260405.md`、`docs/plan/project_roadmap_20260330.md`、`docs/plan/workspace_cleanup_plan_20260405.md`、`docs/plan/chrombpnet_official_externalization_design_20260406.md`、`docs/plan/chrombpnet_official_externalization_implementation_plan_20260406.md` |
...
| 6000 `NT v2 teacher-distill short10`（P0） | 进行中 | 截至 `2026-04-09 21:30 CST`，A6000 双卡当前真实活跃 run 已切到 `ntv2_teacher_distill_short10_s42_6000_20260409_r2`。fresh evidence：`nvidia-smi` 对应两条活跃 python 进程 `pid=862231/862232`；命令行直接指向 `transchrombp.training.train_ddp`、`transchrombp_teacher_v2_center_pool_ntv2_distill.yaml` 与 `teacher_cache_dir=/data1/zhoujiazhen/bylw_atac/TransChromBP/outputs/teacher_cache/ntv2_tutorial_teacher_v1`；日志 `/data1/zhoujiazhen/bylw_atac/TransChromBP/logs/ntv2_teacher_distill_short10_s42_6000_20260409_r2.log` 已推进到 `epoch=1 step=3380/11809`。按本地 `dual-track-20260409` 配置里的 `max_epochs: 10` 粗估，预计完成时间窗约 `2026-04-09 23:45` 到 `2026-04-10 00:15 CST`。 | 等 short10 gate 收口；若相对 matched no-foundation control 满足既定门槛，则再决定是否升 full，否则直接停表并回写 closeout。 | `reports/repository_status_handoff_20260409.md`、`reports/project_plan_code_review_20260405.md`、`reports/paper_claim_evidence_matrix_20260326.md` |
| 6002 RTX 3080 单卡实验侧线 | 进行中 | 截至 `2026-04-09 21:31 CST`，先前的 `teacher_v2_center_pool_skipprobe_v1_wide_short10_s42_6002_r1` 已收口，不再是 active run；当前 `6002` 的 3080 已空闲，`ps -ef | grep transchrombp.training.train_ddp` 也没有活跃训练进程。新的 `U-Net-lite readout` 方向只在 `dual-track-20260409` worktree 中完成了本地代码、配置与 launcher，尚未远端起跑。 | 等 `6000 teacher-distill short10` 给出 gate 结论后，再判断是让 `6002` 正式起 `U-Net-lite`，还是直接把 3080 readout 新 family 一并停表。 | `reports/repository_status_handoff_20260409.md`、`vendor/transchrombp/transchrombp/configs/model/transchrombp_teacher_v2_center_pool_skipprobe_v1_wide.yaml`、`vendor/transchrombp/transchrombp/configs/model/transchrombp_teacher_v2_center_pool_msdec_v1_s2.yaml` |
...
| AI 指令体系 + Git 主档案清理 | 进行中 | `AGENTS.md` 主规范、foundation cache contract 重构、以及 repo-role 收口已经完成主要阶段；当前仍剩两类 cleanup 尾项：`tmp_remote_edit/TransChromBP/scripts` 的 11 个历史 helper script，以及本仓对本地官方源码树的残余活依赖。 | 先清 11 个历史 helper script，再按 `chrombpnet_official_externalization_implementation_plan_20260406` 继续切断残余活依赖。 | `docs/plan/git_cleanup_and_archive_plan_20260321.md`、`docs/plan/workspace_cleanup_plan_20260405.md`、`docs/plan/chrombpnet_official_externalization_design_20260406.md`、`docs/plan/chrombpnet_official_externalization_implementation_plan_20260406.md` |
