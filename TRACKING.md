...
> active local canonical naming/entrypoint 与物理目录都已切换为 `/home/zhengwei/project/python/TransChromBP`；旧路径 `/home/zhengwei/project/python/chromBPNet` 现在只保留为指向新根的 compatibility symlink / rollback path。本轮 rename 审计与验证见 [reports/closeout/local_repo_rename_closeout_20260419.md](reports/closeout/local_repo_rename_closeout_20260419.md)。
> 当前没有 active 的 `6000` training run。最近一次 terminal evidence 统一看 [docs/experiments/registry.md](docs/experiments/registry.md) 和 [docs/experiments/runs.csv](docs/experiments/runs.csv)。
...
> 双机实验规则源统一看 [docs/plan/dual_machine_experiment_charter_20260409.md](docs/plan/dual_machine_experiment_charter_20260409.md)。
> 当前仓库 / worktree / 双机运行快照统一看 [reports/handoff/repository_status_handoff_20260409.md](reports/handoff/repository_status_handoff_20260409.md)；本节只保留结论级 live 状态。
...
|---|---|---|---|---|
| 动态多 loss 协调外发专题包（2026-04-16） | 待验证 | `reports/external/chatgpt_bundle_loss_balance_20260416/` 已成包，问题已收紧到“动态多 loss 协调是否值得迁入当前 loss contract”。 | 按 `00/10` prompt 发给 `GPT Pro`，拿到外部建议后再决定是否立新 hypothesis。 | `reports/external/chatgpt_bundle_loss_balance_20260416/00_readme_upload_order_and_prompt.md`、`reports/external/chatgpt_bundle_loss_balance_20260416/10_ready_to_send_chatgpt_pro_prompt.md` |
| loss balance curriculum | 待处理 | selector-only 与 dynamic-count seed42 都已收口，当前高层结论是“动态器在现有 contract 下没有 clear gain”。 | 若继续这条线，先决定是否进入静态 safe-envelope；否则保持 idle。 | `docs/experiments/registry.md`、`reports/closeout/loss_balance_dynamic_count_seed42_closeout_20260418.md` |
| AlphaGenome-like factor ladder（E1/E2/E3） | 待处理 | queue 停在 `export_e2_teacher_cache`；根因是 exporter CLI 与 `--targets profile16 logcount` 参数漂移。 | 对齐 exporter CLI，补最小 smoke；未修复前不要继续推进 E3。 | `docs/plan/alphagenome_like_factor_ladder_design_20260411.md`、`docs/plan/alphagenome_like_factor_ladder_implementation_20260411.md`、`reports/analysis/unet_vs_alphagenome_reassessment_20260411.md` |
| 官方 ChromBPNet 外置化残项 | 待处理 | canonical official root 已稳定为 `/data1/zhoujiazhen/bylw_atac/chromBPNet`；bridge smoke、closeout、patch ledger 已齐。 | 只清理残留 `chrombpnet_official` 旧写法；不再维护 live mounted worktree。 | `reports/closeout/chrombpnet_official_externalization_closeout_20260408.md`、`reports/governance/chrombpnet_official_patch_ledger_20260406.md` |
| 论文主稿与 supporting writeup | 进行中 | paper-facing 口径已经收束到 `bias-safe framework + full/debiased diagnostics + stable readout`。 | 继续同步英文 draft / supplementary，并压实 `L3 shared-region` 与分类附表表述。 | `reports/paper/transchrombp_paper_cn_v1.tex`、`reports/paper/transchrombp_paper_draft_v1.tex`、`reports/paper/paper_claim_evidence_matrix_20260326.md` |
| 文档体系重整与历史归档收口（2026-04-19） | 待验证 | `docs/plan/`、`reports/`、`docs/learning/`、`docs/research/` 已按新结构重排；`README.md` 与目录入口已更新。 | 做一轮链接 / reader pass，确认无断链、无重复入口、无误把 archive 当 live root 的残留。 | `README.md`、`docs/learning/学习材料索引.md`、`reports/README.md` |
| foundation models 下载完整性验证 | 进行中 | `Genos-1.2B` 已完成本地加载与前向验证；其余 foundation model 继续按“最小加载可用性”原则核验。 | 网络恢复后优先补 `Genos` 的 HF 连通性 / Docker 兜底，其余模型按当前优先级推进。 | `docs/env/transchrombp_genos_env.md`、`docs/archive/ops/nucleotide-transformer_download_turol.md` |
