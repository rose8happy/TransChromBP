...
|---|---|---|---|---|
| 仓库任务盘点（2026-04-09） | 进行中 | 截至 `2026-04-10 00:22:55 CST`，本轮 worktree cleanup 仍保持收口态：`master` 是 clean 的 canonical 档案仓，`dual-track-20260409 -> 4ffc6e7`、`autonomy/20260406-structure -> 764b8c0`、`autonomy/20260406-chrombpnet-externalization -> 4aadffa` 继续作为冻结支线保留。实验面上的新增事实是：`6000` 已完成 `alphagenome_matched_raw_track_slice_v1_20260410`，并以 technical/alignment gate `pass` 收口；4 个固定 loci 全部成功，`summary / metadata / profiles / run_meta / merged` 产物齐全，这条线不再处于“候选 backlog”或“待启动”状态。`6002` 未在本轮复核，仍按各自 closeout 线单独处理。 | `6000` 若继续 AlphaGenome 方向，唯一允许下一步是扩到约 `12-20 loci` 的稍大 matched panel，仍定位为窄 external coordinate；不要把本次 `pass` 改写成模型质量提升。`6002` 继续按自己的 rigor closeout 文档收口。 | `docs/plan/2026-04-09_dual_machine_experiment_charter.md`、`reports/alphagenome_matched_raw_track_slice_v1_closeout_20260410.md`、`reports/unet_lite_v1_rigor_review_20260409.md`、`reports/repository_status_handoff_20260409.md` |
...
| 6000 AlphaGenome matched raw-track slice | 待处理 | 截至 `2026-04-10 00:22:55 CST`，`alphagenome_matched_raw_track_slice_v1_20260410` 已完成 closeout 并判定为 technical/alignment gate `pass`。已复核的硬证据包括：启动时间 `2026-04-10 00:22:40 CST`；日志尾部出现 `Wrote summary`、`Wrote metadata`、`Wrote merged comparison`；输出目录含 `summary.csv`、`region_metadata.jsonl`、`profiles/*.npz`、`run_meta.json`、`merged_locus_totals.csv`；`summary.csv`/`region_metadata.jsonl` 均为 `4` 行，4 个固定 loci 全部成功，且每个位点 `num_tracks_used=1`。这说明 `6000` 的窄 external-coordinate pilot 已从“待发车 backlog”转成“已完成的可复用基线”。 | 若继续该线，唯一允许下一步是扩到约 `12-20 loci` 的稍大 matched panel，仍然只做窄 external coordinate，不升格成大 benchmark，也不据此宣称模型质量提升。 | `docs/plan/2026-04-09_dual_machine_experiment_charter.md`、`reports/alphagenome_matched_raw_track_slice_v1_closeout_20260410.md`、`docs/superpowers/specs/2026-04-10-a6000-alphagenome-matched-slice-v1-design.md` |
