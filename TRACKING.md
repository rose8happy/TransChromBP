...
|---|---|---|---|---|
| 仓库任务盘点（2026-04-09） | 进行中 | 截至 `2026-04-09 22:43 CST`，本轮 worktree cleanup 已完成：`master` 已恢复为 clean 的 canonical 档案仓，并用本轮 docs closeout 收口主档案；实验/工程支线也已各自冻结为 clean worktree + 本地提交：`dual-track-20260409 -> 4ffc6e7 (wip: snapshot dual track pivot)`、`autonomy/20260406-structure -> 764b8c0`、`autonomy/20260406-chrombpnet-externalization -> 4aadffa`。当前 live 实验面按新 charter 收口为：`6000 teacher-distill short10 r2` 已 early-stop 收口，正在等正式 gate 判读；`6002` 则继续跑确认性 cheap rerun `teacher_v2_center_pool_unet_lite_v1_short10_s42_6002_20260409_r4`，用来确认 `r3` 的 `provisional no-go` 是否稳定。 | 按 charter 先做两件事：先判 `6000 teacher-distill short10 r2` 是否过 gate，再等 `6002 U-Net-lite r4` 收口；随后把 `v1` 正式收成“可停表”还是“仅 single-run no-go”。在白名单队列没收口前，不默认切回“先写论文”。 | `docs/plan/2026-04-09_dual_machine_experiment_charter.md`、`reports/unet_lite_v1_rigor_review_20260409.md`、`reports/repository_status_handoff_20260409.md`、`reports/project_plan_code_review_20260405.md`、`docs/plan/project_roadmap_20260330.md` |
...
| 6000 `NT v2 teacher-distill short10`（P0） | 待验证 | 截至 `2026-04-09 22:43 CST`，A6000 双卡对应的 `ntv2_teacher_distill_short10_s42_6000_20260409_r2` 已确认收口：`nvidia-smi` 已回到 `13 MiB / 0% util`，相关训练进程已退出；日志尾部明确写出 `[early-stop] epoch=5` 与 `NT v2 teacher distill completed.`。fresh evidence 还包括：`run_meta.json` 已落盘，记录 `best_epoch=2`、`best_metric_name=peak.profile_target_jsd_full_mean`、`best_metric_value=0.345324...`；log 里对应 `epoch=2` 的 `val:peak` 为 `profile_target_jsd_full_mean=0.3453`、`count_pearson_full=0.5781`。这说明 run 已完成，但 gate 判读还没正式写死。 | 先按 matched `short10_nofoundation_control` 做正式 gate 判读；若仍按当前 best 读数成立，则这条线大概率不会升 `full-budget distill`，但在 closeout 文档写死前先不口头替代正式结论。 | `docs/plan/2026-04-09_dual_machine_experiment_charter.md`、`reports/repository_status_handoff_20260409.md`、`reports/project_plan_code_review_20260405.md`、`reports/paper_claim_evidence_matrix_20260326.md` |
| 6002 RTX 3080 单卡实验侧线 | 进行中 | 截至 `2026-04-09 22:41 CST`，`6002` 的 3080 已重新被占用：确认性 cheap rerun `teacher_v2_center_pool_unet_lite_v1_short10_s42_6002_20260409_r4` 已启动，目的不是扩线，而是验证 `r3` 的 `provisional no-go` 是否稳定。fresh evidence：GPU `100% util / 4287 MiB`；活跃训练进程已指向 `train_tutorial_teacher_v2_readout_short10_teacher_v2_center_pool_unet_lite_v1_short10_s42_6002_20260409_r4_runtime.yaml`、`data_tutorial_canonical_v1_6002.yaml` 与同一 `unet_lite_v1` model config；日志 `/home/zhengwei/bylw_atac/TransChromBP/logs/teacher_v2_center_pool_unet_lite_v1_short10_s42_6002_20260409_r4.log` 已越过 header，进入真实 training setup。关键配置差异相对 `r3` 只改了 `run_name` 与 `MASTER_PORT_BASE=30063`，其余 seed / batch / data config 保持同口径。按 `r3` 的真实节奏粗估，预计完成时间窗约 `2026-04-10 00:50` 到 `2026-04-10 01:30 CST`。 | 盯首个有效 validation 与最终 best：若 `r4` 仍落在与 `r3` 类似区间，就把 `U-Net-lite v1` 正式收成可停表 verdict；若明显改善，再重新评估它是否保留在 3080 shortlist。监控命令：`tail -f /home/zhengwei/bylw_atac/TransChromBP/logs/teacher_v2_center_pool_unet_lite_v1_short10_s42_6002_20260409_r4.log`。 | `docs/plan/2026-04-09_dual_machine_experiment_charter.md`、`reports/unet_lite_v1_rigor_review_20260409.md`、`reports/repository_status_handoff_20260409.md` |
