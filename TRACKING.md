...
|---|---|---|---|---|
| AlphaGenome-like factor ladder（E1/E2/E3） | 进行中 | `alphagenome_factor_ladder` 的 docs/spec/plan 已在 canonical repo 注册；Task 1-5 runtime 实现当前位于 `6000/6002` remote isolated workdirs，canonical archive 代码快照尚未回填；正式 run 尚未启动。 | 下一步仍只允许一条小规模 remote smoke：优先 `exporter+distill smoke`，或最小 `hierdec4096` launcher smoke；暂不准直接开正式 run。 | `docs/superpowers/specs/2026-04-11-alphagenome-like-factor-ladder-design.md`、`reports/unet_vs_alphagenome_reassessment_20260411.md`、`docs/experiments/registry.md` |
