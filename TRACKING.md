...
> active local canonical naming/entrypoint 已切换为 `/home/zhengwei/project/python/TransChromBP`；当前 filesystem reality 仍是 `/home/zhengwei/project/python/chromBPNet` 作为旧物理目录存在，而 `/home/zhengwei/project/python/TransChromBP -> chromBPNet` 是 pre-rename 阶段的临时 forward bridge；等真实物理 rename 完成后，旧路径才会变成兼容 symlink / rollback path。当前这一阶段的 canonical 审计闭环见 [reports/local_repo_rename_closeout_20260419.md](reports/local_repo_rename_closeout_20260419.md)，真实物理 rename 是下一步。
...
|---|---|---|---|---|
| 本地仓物理 rename 过渡 | 待处理 | repository-local absolute links 已重写；当前临时 forward symlink `/home/zhengwei/project/python/TransChromBP -> chromBPNet` 仍在为旧链接兜底；pre-rename 审计主档案已写成 [reports/local_repo_rename_closeout_20260419.md](reports/local_repo_rename_closeout_20260419.md)。 | 执行真实物理 rename，并在完成后清理临时 forward bridge、建立旧路径兼容 symlink、复跑治理验证。 | `reports/local_repo_rename_closeout_20260419.md` |
...
| 官方 ChromBPNet 外置化实施 | 进行中 | 截至 `2026-04-19`，`6000` 的真实 bridge smoke `chrombpnet_official_step3_bridge_smoke_20260408_110411` 仍是这条 infra 线的 terminal evidence，且 canonical closeout / patch ledger / closeout tag 已补齐。当前可以确认的口径是：helper bridge 已在真机验证通过，唯一已验证的官方根路径就是 `/data1/zhoujiazhen/bylw_atac/chromBPNet`；implementation plan 与 patch ledger 现在只分别承担“怎么做”和“依赖哪些官方触点”，不再兼任结果报告。mounted worktree 现在已经不是必需入口，如需恢复现场可从 branch 或 snapshot/closeout tag 回放。 | 若继续收这条线，下一步只剩清理少数历史文档里残留的 `chrombpnet_official` 旧写法；不需要再保留 live mounted worktree。 | `reports/chrombpnet_official_externalization_closeout_20260408.md`、`reports/chrombpnet_official_patch_ledger_20260406.md`、`docs/plan/chrombpnet_official_externalization_design_20260406.md`、`docs/plan/chrombpnet_official_externalization_implementation_plan_20260406.md` |
...
| AI 指令体系 + Git 主档案清理 | 进行中 | 在先前 `master`/worktree 收口的基础上，本轮继续把剩余 archival branch 的入口压平：`reports/dual_track_pivot_snapshot_20260409.md` 与 `reports/foundation_cache_contract_closeout_20260419.md` 已把 `dual-track-20260409`、`autonomy/20260406-structure` 的 master-side closeout 补齐；`chrombpnet_externalization` 已补 `closeout/chrombpnet_externalization/20260408` tag 和最后一跳链接修正；三条 branch 现在都改成通过 branch + snapshot/closeout tag 恢复，不再依赖 live mounted worktree。active local canonical root 现统一表述为 `/home/zhengwei/project/python/TransChromBP`，旧 `/home/zhengwei/project/python/chromBPNet` 仅保留兼容/回滚语义。 | 剩余减噪主要是历史报告里的绝对路径批量改写；live/canonical 文档不再把旧本地路径当成当前入口。 | `docs/env/repository_governance.md`、`reports/repository_governance_cleanup_20260419.md`、`reports/worktree_registry_audit_20260419.md`、`reports/project_broad_audit_cleanup_20260419.md`、`reports/dual_track_pivot_snapshot_20260409.md`、`reports/foundation_cache_contract_closeout_20260419.md` |
...
2. 自动化同步脚本：历史上这里先落了 `scripts/sync_project.sh` 的初版；当前标准入口已收敛为 `publish-runtime-{6000,6002}`、`pull-results-{6000,6002}` 与 `status-all`，旧 `deploy` / `download_results` 只保留为 deprecated 迁移提示。
