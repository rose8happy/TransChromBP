...
> active local canonical naming/entrypoint 与物理目录都已切换为 `/home/zhengwei/project/python/TransChromBP`；旧路径 `/home/zhengwei/project/python/chromBPNet` 现在只保留为指向新根的 compatibility symlink / rollback path。本轮 rename 审计与验证见 [reports/local_repo_rename_closeout_20260419.md](reports/local_repo_rename_closeout_20260419.md)。
...
| AI 指令体系 + Git 主档案清理 | 进行中 | 在先前 `master`/worktree 收口的基础上，本轮继续把剩余 archival branch 的入口压平：`reports/dual_track_pivot_snapshot_20260409.md` 与 `reports/foundation_cache_contract_closeout_20260419.md` 已把 `dual-track-20260409`、`autonomy/20260406-structure` 的 master-side closeout 补齐；`chrombpnet_externalization` 已补 `closeout/chrombpnet_externalization/20260408` tag 和最后一跳链接修正；三条 branch 现在都改成通过 branch + snapshot/closeout tag 恢复，不再依赖 live mounted worktree。本地 canonical physical root 现已完成 `/home/zhengwei/project/python/TransChromBP`，旧 `/home/zhengwei/project/python/chromBPNet` 只保留为 compatibility symlink / rollback path；rename 审计见 `reports/local_repo_rename_closeout_20260419.md`。 | 若继续减噪，优先清理更多历史报告里的旧本地绝对路径；路径层面只剩兼容期结束时决定何时移除 `chromBPNet -> TransChromBP` symlink。 | `docs/env/repository_governance.md`、`reports/repository_governance_cleanup_20260419.md`、`reports/worktree_registry_audit_20260419.md`、`reports/project_broad_audit_cleanup_20260419.md`、`reports/dual_track_pivot_snapshot_20260409.md`、`reports/foundation_cache_contract_closeout_20260419.md` |
