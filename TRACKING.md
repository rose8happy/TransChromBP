...
|---|---|---|---|---|
| 仓库任务盘点（2026-04-09） | 进行中 | 截至 `2026-04-09 22:46 CST`，本轮 worktree cleanup 已完成：`master` 已恢复为 clean 的 canonical 档案仓，并用本轮 docs closeout 收口主档案；实验/工程支线也已各自冻结为 clean worktree + 本地提交：`dual-track-20260409 -> 4ffc6e7 (wip: snapshot dual track pivot)`、`autonomy/20260406-structure -> 764b8c0`、`autonomy/20260406-chrombpnet-externalization -> 4aadffa`。当前 live 实验面按新 charter 收口为：`6000 teacher-distill short10 r2` 已正式判负并停表，不升 `full-budget distill`；`6002` 则继续跑确认性 cheap rerun `teacher_v2_center_pool_unet_lite_v1_short10_s42_6002_20260409_r4`，用来确认 `r3` 的 `provisional no-go` 是否稳定。 | 按 charter 继续等 `6002 U-Net-lite r4` 收口；随后把 `v1` 正式收成“可停表”还是“仅 single-run no-go”。若 `6002` 也不给正结果，再讨论下一个 genuinely new dense decoder / readout 变体，或转 `AlphaGenome matched raw-track slice`。 | `docs/plan/2026-04-09_dual_machine_experiment_charter.md`、`reports/ntv2_teacher_distill_short10_gate_closeout_20260409.md`、`reports/unet_lite_v1_rigor_review_20260409.md`、`reports/repository_status_handoff_20260409.md` |
...
| 6000 `NT v2 teacher-distill short10`（P0） | 进行中 | 截至 `2026-04-09 22:46 CST`，A6000 双卡对应的 `ntv2_teacher_distill_short10_s42_6000_20260409_r2` 已完成 formal gate closeout。使用 matched `short10_nofoundation_control` 的同口径 `valid peak` 做对照后，结论非常明确：control best `val:peak` 为 `0.336346 / 0.8006`，而 distill `r2` best `val:peak` 只有 `0.345324 / 0.5781`，既没有达到 `peak JSD` 改善 `>=0.002`，也没有满足 `count_r` 不下降超过 `0.005`；相反，`JSD` 变差约 `+0.009`，`count_r` 下降约 `-0.223`。因此这条线按原 stop-rule 正式判负：不升 `full-budget distill`，不扩 seed，tutorial teacher-distill 线当前停表。 | 不再给这条 tutorial `teacher-distill` 线分配 `6000` 后续 run；当前默认继续等 `6002 U-Net-lite r4` 收口，并把下一次 6000 占卡资格留给后续真的过了 3080 shortlist 的方向。 | `docs/plan/2026-04-09_dual_machine_experiment_charter.md`、`reports/ntv2_teacher_distill_short10_gate_closeout_20260409.md`、`reports/repository_status_handoff_20260409.md` |
