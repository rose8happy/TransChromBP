...
| 6002 Genos 无关独立实验线（GM12878/K562） | 进行中 | `G_s42=profref` 已收口；`GM12878` 五个关键文件、data/train config 与 launcher 均已齐全。`gm12878_v2_6002_smoke` 的 `Invalid interval bounds` 根因已定位为 nonpeak BED 含 13 条 `bigWig` 缺失染色体记录（`chrM`×1、`chrUn_KI270302v1`×1、`chrUn_GL000226v1`×4、`chrY_KI270740v1_random`×7），并已在本地主档案与 6002 运行目录的 `real_data.py` 加入按 `bigWig` 染色体集合的过滤/防护；`GM12878/K562` 的 6002 data config fallback `sampling.nonpeak_ratio` 也已对齐到 `0.1`。修复后远端已通过 `py_compile + dataset 构造 + 多样本 __getitem__` 验证。`gm12878_v2_6002_smoke_retry_20260322_003645` 已于 2026-03-22 03:56 CST 在 `epoch 7` early-stop 收口，`best epoch=3`、`best peak.profile_target_jsd_full_mean=0.4351`，最后一次验证为 `val:peak profile_target_jsd_full_mean=0.4371`、`count_pearson_full=0.7970`。随后 `k562_v2_6002_smoke_20260322_161602` 已于 17:55 CST 按 `max_epochs=10` 正常收口；`best epoch=9`、`best peak.profile_target_jsd_full_mean=0.6331`，最后一次验证为 `val:peak profile_target_jsd_full_mean=0.6354`、`count_pearson_full=0.8411`。当前已按既定队列在 2026-03-22 19:17 CST 于 6002 后台启动 `gm12878_v2_6002_pilot20_20260322_191746`；短检查已进入 `epoch 1 step 140/31482`，GPU 利用率约 `97%`、显存约 `2.0 GiB`。启动期日志里的 `RuntimeWarning` 仅表示按 `bigWig` 染色体集合过滤了 `13` 条 `GM12878` nonpeak 不匹配记录（`chrM`、`chrUn_GL000226v1`、`chrUn_KI270302v1`、`chrY_KI270740v1_random`），训练链路未被中断 | 继续盯 `gm12878_v2_6002_pilot20_20260322_191746` 的首轮 validation、`best.pt` 和 `epoch_metrics.jsonl` 是否正常写出；待 pilot 进入关键验证节点后，再决定是否继续补 `K562 20ep pilot` 或做 held-out 评估整理 | `docs/plan/6002_independent_experiment_queue_20260321.md`、`tmp_remote_edit/TransChromBP/scripts/launch_v2fix_6002_single_dataset.sh`、`tmp_remote_edit/transchrombp/data/real_data.py`、`tmp_remote_edit/transchrombp/configs/data/data_gm12878_canonical_v1_6002.yaml`、`tmp_remote_edit/transchrombp/configs/data/data_k562_canonical_v1_6002.yaml`、`tmp_remote_edit/transchrombp/configs/train/train_gm12878_v2_6002_pilot20.yaml`、`/home/zhengwei/bylw_atac/logs/gm12878_v2_6002_smoke_retry_20260322_003645.log`、`/home/zhengwei/bylw_atac/logs/gm12878_v2_6002_smoke_retry_20260322_003645.launch.log`、`/home/zhengwei/bylw_atac/logs/k562_v2_6002_smoke_20260322_161602.log`、`/home/zhengwei/bylw_atac/logs/k562_v2_6002_smoke_20260322_161602.launch.log`、`/home/zhengwei/bylw_atac/logs/gm12878_v2_6002_pilot20_20260322_191746.log`、`/home/zhengwei/bylw_atac/logs/gm12878_v2_6002_pilot20_20260322_191746.launch.log` |
...
| Genos 融合实验设计与 Phase 0 快速验证 | 进行中 | Phase 0 结论稳定：唯一候选层仍是 `layer_6`，Phase 1 只能做最小 frozen pilot。2026-03-21 已完成起跑前脚本/配置修正并在 6000 重启主线；`G1` quick batch sweep 也已收口，`bs=8/12/16/20` 全部成功，峰值显存约 `5.4/6.4/7.2/8.1 GiB`，50-step walltime 为 `35/47/58/71 s`。截至 2026-03-22 19:12 CST，`G0(batch=20)` 已于 2026-03-21 18:52 CST 在 `epoch 20/20` 收口，最后一次 `val:peak profile_target_jsd_full_mean=0.3337`、`count_pearson_full=0.7998`，`best epoch=20`。`G1(batch=20)` 仍在 GPU1 上运行，最近标准输出日志刷新到 `epoch 8 step 3380/11809`、`loss=16.99292`、`lr=3.933e-04`；进程 `PID 1825216` 仍在跑，`ps` 显示 `%CPU≈112`，`nvidia-smi pmon` 观察到 GPU1 `sm≈83%`、显存约 `8.1 GiB`。`G2(batch=20)` 也在 GPU0 上正常推进，最近标准输出日志刷新到 `epoch 5 step 4980/11809`、`loss=17.04148`、`lr=4.682e-04`；进程 `PID 1889714` 仍在跑，`ps` 显示 `%CPU≈112`，`nvidia-smi pmon` 观察到 GPU0 `sm≈67%`、显存约 `8.1 GiB`，不再是此前“日志长时间不刷新”的状态。两条 run 的 `epoch_metrics.jsonl` 最近仍只记录到 `G1 epoch 7` / `G2 epoch 4`，说明下一次 validation 尚未落盘。同机另有 `ningjiaxin` 的 `toolrl_grpo_qwen_20260322` 占用两张 A6000，GPU0/1 额外占用约 `17.0/11.5 GiB` | 继续盯 `G1/G2` 的下一次 validation，重点看 `best.pt` 是否刷新以及 `peak.profile_target_jsd_full_mean` 相对 `G0` 的增益；待任一条进入新的验证节点后，再决定是否释放 6000 训练窗口给 validation 指标改造写回 | `docs/plan/genos_phase1_rebase_checklist.md`、`docs/plan/genos_phase1_minimal_integration_plan.md`、`docs/plan/experiment_arrangement_20260321.md`、`tmp_remote_edit/transchrombp/scripts/run_genos_pilot.sh`、`tmp_remote_edit/transchrombp/scripts/run_genos_batch_sweep.sh`、`tmp_remote_edit/transchrombp/configs/train/train_v2fix_genos_profile_select.yaml`、`/data1/zhoujiazhen/bylw_atac/logs/genos_G0_20260321_104549.launch.log`、`/data1/zhoujiazhen/bylw_atac/logs/genos_20260321_baseline_s42.log`、`/data1/zhoujiazhen/bylw_atac/logs/genos_G1_20260321_launch.log`、`/data1/zhoujiazhen/bylw_atac/logs/genos_20260321_gate_s42.log`、`/data1/zhoujiazhen/bylw_atac/logs/genos_G2_20260322_004704.launch.log`、`/data1/zhoujiazhen/bylw_atac/logs/genos_20260322_mean_s42.log`、`/data1/zhoujiazhen/bylw_atac/logs/genos_batch_sweep_20260321_100046.log`、`/data1/zhoujiazhen/bylw_atac/TransChromBP/outputs/logs/genos_batch_sweep_20260321_100046.tsv` |
