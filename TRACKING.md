...
> active local canonical root 已切换为 `/home/zhengwei/project/python/TransChromBP`；旧 `/home/zhengwei/project/python/chromBPNet` 若仍存在，只作为兼容 symlink / rollback path，不再作为 canonical entrypoint。当前还保留临时 forward symlink `/home/zhengwei/project/python/TransChromBP -> chromBPNet`，用于 pre-rename 阶段避免 dead links；这一阶段的 canonical 审计闭环见 [reports/local_repo_rename_closeout_20260419.md](reports/local_repo_rename_closeout_20260419.md)，真实物理 rename 是下一步。
