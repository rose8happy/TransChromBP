...
|---|---|---|---|---|
| AlphaGenome-like factor ladder（E1/E2/E3） | 进行中 | `alphagenome_factor_ladder` 的 docs/spec/plan 已在 canonical repo 注册；`6000` remote isolated workdir 现已在 commit `5877077` 上完成一条真实 single-GPU `exporter+distill` smoke `hierdec4096_exporter_distill_smoke_20260411_2110`（`32 train / 32 val`, `dry_run_steps=2`），打通了“最小 teacher checkpoint -> teacher cache export -> distill dry-run”整条链路。bring-up 期间两条更早 smoke 也已定位清楚：`2058` 因 runtime 无条件透传 `genos_summary` 给 hierdec fail，`2105` 因 smoke train config 未同步缩小 `max_train_regions/max_val_regions` 导致 teacher cache `n_records` mismatch；前者已作为 remote runtime 修补提交并同步到 `6002`，后者已收进一次性 smoke config。canonical archive 代码快照仍尚未回填；正式 short10 / formal run 仍未启动。 | 下一步优先把 remote runtime 变更回填到 canonical archive，再决定是否直接启动首条正式 `E2/E3 short10` gate；同类 technical smoke 暂不需要继续追加。 | `docs/superpowers/specs/2026-04-11-alphagenome-like-factor-ladder-design.md`、`reports/unet_vs_alphagenome_reassessment_20260411.md`、`docs/experiments/registry.md` |
