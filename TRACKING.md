...
| `NT v2 teacher-distill gate`（6000 / A6000×2） | 进行中 | `2026-04-09 21:25 CST` 已完成本地修复回归并同步到 6000，然后重启同一条 gate：run name `ntv2_teacher_distill_short10_s42_6000_20260409_r2`，launcher PID `861736`，日志 `/data1/zhoujiazhen/bylw_atac/TransChromBP/logs/ntv2_teacher_distill_short10_s42_6000_20260409_r2.log`。随后再次复核时已确认这条 `r2` 顺利越过旧 bug：Step 0 直接跳过，Step 1 `teacher_cache_export` 已成功写出 `teacher_manifest_{train,valid,test}.json` 与三组 `profile16/logcount` `.npy`，并已进入 Step 2 双卡 short10 训练。最新日志已出现真实 distill 训练行：`epoch=1 step=280/11809 loss=20.9759`，且 `loss_distill_profile/loss_distill_count` 所依赖的 teacher cache 已经在位，因此当前不再是“链路修复待验证”，而是正常 short10 gate 在跑。按现有首轮速率粗估，单 epoch 约 `10-15` 分钟；若速率稳定、早停不明显拖慢，收口时间窗更可能落在 `2026-04-10 00:20-01:00 CST`。 | 继续盯首个有效 validation，重点看 `peak.profile_target_jsd_full_mean`、`peak.count_pearson_full` 以及 `loss_distill_profile/loss_distill_count` 是否保持健康下降；收口后立即与 matched `short10_nofoundation_control` 做 gate 判读，决定是否升 full-budget。 | `docs/plan/dual_track_ntv2_distill_unet_lite_20260409.md`、`reports/dual_track_experiment_pivot_20260409.md`、`vendor/transchrombp/transchrombp/evaluation/teacher_cache_export.py`、`vendor/transchrombp/transchrombp/scripts/run_ntv2_teacher_distill.sh` |
| `U-Net-lite readout probe`（6002 / RTX 3080） | 待验证 | `2026-04-09 21:16 CST` 再查日志时，`teacher_v2_center_pool_unet_lite_v1_short10_s42_6002_20260409_r3` 已在 `epoch=5` 触发 early-stop 并正常收口，launcher 已写出 `run_meta.json`，GPU 回到 `27 MiB / 0%`。best checkpoint 出现在 `epoch=2`，peak valid 为 `profile_target_jsd_full_mean=0.455245`、`count_pearson_full=0.7711`；最终 `epoch=5` 的 peak valid 为 `0.455784 / 0.7726`。与现有 `skipprobe_v1_wide` shortlist `0.4494 / 0.8075` 相比，这个 `U-Net-lite v1` 当前在 `JSD` 和 `count_r` 两侧都更差；相对 matched `short10_nofoundation_control` 的 peak `0.3193 / 0.8298` 也仍明显落后，因此现阶段更像是又一条负向 cheap-screen，而不是可晋级候选。 | 把这条 run 的 gate 判读正式写回比较文档；若没有额外证据推翻当前对照，`U-Net-lite v1` 线按 stop-rule 直接停表，不升到 `6000`，也不扩 seed。 | `docs/plan/dual_track_ntv2_distill_unet_lite_20260409.md`、`reports/dual_track_experiment_pivot_20260409.md`、`vendor/transchrombp/transchrombp/models/profile_decoder.py`、`vendor/transchrombp/transchrombp/scripts/run_unet_lite_decoder_probe.sh` |
...
| A6000 利用率 + foundation-model 停表规则 | 进行中 | `2026-04-09` 实时复核时，6000 两张 A6000 仍是 `13 MiB / 0% util`。旧结论没有变化：`NT v2 residual short10 family` 默认停表，`bins4/coarse residual` 与 `bins16 residual` 都已判负。当前唯一被显式放行的新占卡路线，是把 foundation 改成 offline teacher 的 `NT v2 distill_only` short10 gate；这不再属于当前 residual family 的自然延长。 | 默认不再为 residual family 分配 A6000；若继续使用双卡，优先执行 `NT v2 teacher-distill short10`。只有 gate 通过才升 full-budget，否则直接停表。 | `docs/plan/post_chatgpt_pro_priority_execution_20260405.md`、`docs/plan/a6000_foundation_reentry_candidates_20260405.md`、`docs/plan/dual_track_ntv2_distill_unet_lite_20260409.md`、`reports/foundation_model_restart_v3_implementation_20260405.md` |
| `short10 matched no-foundation control`（6000 / A6000×2） | 进行中 | full held-out 已完成并写出 `/data1/zhoujiazhen/bylw_atac/TransChromBP/outputs/metrics/short10_nofoundation_control_s42_20260405_dual_best_test_full_20260405_2100.json`；日志 `/data1/zhoujiazhen/bylw_atac/logs/short10_nofoundation_control_testfull_20260405_6000.log` 已收口。核心结果：`n=62342`，`count_pearson_full=0.8457`，`count_pearson_debiased=0.8419`，`profile_target_jsd_full_mean=0.4395`，`profile_pearson_full_mean=0.7116`，分类指标 `auroc=0.8677`、`auprc=0.8647`、`f1=0.7902`。这条 run 已经回答了关键问题：`short10` 预算本身仍能维持健康 baseline，因此它现在是固定校准底座，而不是待继续验证的实验。 | 继续把这条 matched baseline 当作 `NT v2 teacher-distill` 与 3080 新 readout cheap-screen 的对照底座，不再单独重跑。 | `docs/plan/a6000_foundation_reentry_candidates_20260405.md`、`docs/plan/dual_track_ntv2_distill_unet_lite_20260409.md`、`vendor/transchrombp/transchrombp/scripts/run_short10_no_foundation_control.sh` |
...
| foundation model gate 总结（tutorial canonical） | 进行中 | 当前 gate 已进一步收紧：`Genos cached P0/P1` 为负结果，`Caduceus-PS` 单 seed A/B 为 `near-null / marginal positive`，`NT v2 residual` 的 `bins4/coarse residual` 与 `bins16 residual` 都已判负，而 `msdec/skipprobe` readout 小变体也未形成 clean gain。当前仅保留一条新的、边界清楚的重入：`NT v2 -> distill_only teacher`，其 short10 gate 与停表规则已另行写入 `dual_track_ntv2_distill_unet_lite_20260409`。 | foundation 线继续作为受控 side quest；默认不再扩 residual/cross-attention family，而是只执行已经写明 gate 的 `teacher-distill` 短跑。 | `reports/nt_v2_probe_20260404.md`、`reports/chatgpt_pro_external_review_20260405.md`、`docs/plan/a6000_foundation_reentry_candidates_20260405.md`、`docs/plan/dual_track_ntv2_distill_unet_lite_20260409.md` |
...
### 2026-04-09 锁定后续实验顺序（防跑偏）

1. 当前不允许再新开第三条训练线；必须先等 `6000: ntv2_teacher_distill_short10_s42_6000_20260409_r1` 和 `6002: teacher_v2_center_pool_unet_lite_v1_short10_s42_6002_20260409_r3` 收口。
2. `6000` 这条线只有两种合法后续：
   `short10` 过 gate，则下一步且唯一下一步是同配方升 `full-budget distill`；不过 gate，则 `NT v2 teacher-distill` tutorial 线直接停表，不扩 seed、不加 teacher target、不切别的 adapter。
3. `6002` 这条线只有两种合法后续：
   `U-Net-lite short10` 明确优于 matched control / shortlist，则先写比较结论，再决定是否需要一次确认性复现；若没有 clean gain，则 `U-Net-lite` 线直接停表，不自动升到 `6000`，不自动扩 seed。
4. 当前明确禁止自动继续的方向：
   `msdec_v1` 新变体、`skipprobe` 新变体、`NT v2 residual/cross_attention` family、`AlphaGenome/Genos/Caduceus` 新 teacher、`GM12878/K562` 扩线、任何“先开了再说”的 A6000 占卡试验。
5. 只有在两条在跑任务都完成并完成 gate 判读后，才允许讨论新的 genuinely new hypothesis；讨论结果若要落地，必须先把 hypothesis、stop-rule、run name、机器/GPU、日志路径、ETA 写回 `TRACKING.md` 再启动。
6. 默认监控节奏：
   首个有效 validation 时更新一次 `TRACKING.md`；
   run 收口时更新一次 `TRACKING.md`；
   若出现 crash / 资源不足 / 指标明显提前判负，也必须当轮写回，不允许口头记忆。
