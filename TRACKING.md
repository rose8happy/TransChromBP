...
| ATAC 分类指标扩展（AUROC/AUPRC/F1） | 进行中 | `chrombpnet/training/predict.py`、selector 和汇总脚本已闭环支持 peak-vs-nonpeak 分类指标；当前定位是 supplementary 指标，不进入主表。 | 把分类指标补进 strict-compare 附表和论文 supplementary。 | `chrombpnet/training/predict.py`、`reports/tutorial_L3_shared_region_closure_20260330.md` |
