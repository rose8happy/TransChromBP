...
> 当前存在 1 条 active 的 `6000` training run：`teacher_v2_hierdec4096_short10_s42_6000_20260411_r1`（`alphagenome_factor_ladder` / formal `E2 short10`，dual A6000，start `2026-04-11 21:46:19 CST`，log 见 `docs/experiments/runs.csv`）；其余旧 `A6000 formal gate`、`AlphaGenome v2 sidecar` 与 `U-Net-lite v1` 都已收口。
...
|---|---|---|---|---|
| AlphaGenome-like factor ladder（E1/E2/E3） | 进行中 | `alphagenome_factor_ladder` 的 docs/spec/plan 已在 canonical repo 注册；`6000` remote isolated workdir 在 commit `5877077` 上完成的 single-GPU `exporter+distill` smoke `hierdec4096_exporter_distill_smoke_20260411_2110`（`32 train / 32 val`, `dry_run_steps=2`）现已回填进 canonical archive commit `079b603`。archive snapshot 现在包含 `longctx4096` / `hierdec4096` configs、三个 launcher、`HierarchicalTransChromBP`、`model_teacher_cache_export.py`、以及对应本地回归测试。bring-up 期间两条更早 smoke 也都保留在 run manifest：`2058` 暴露 `genos_summary` kwargs gating，`2105` 暴露 smoke train config 的 teacher-cache `n_records` mismatch；这两处都已有明确技术结论。正式 `E2` dual-A6000 gate `teacher_v2_hierdec4096_short10_s42_6000_20260411_r1` 已于 `2026-04-11 21:46:19 CST` 启动，当前 log `/data1/zhoujiazhen/bylw_atac/.codex_jobs/alphagenome_factor_ladder_20260411/logs/teacher_v2_hierdec4096_short10_s42_6000_20260411_r1.log` 已进入 `epoch=1`，两张 A6000 利用率约 `95-97%`，global batch `32`。基于首分钟吞吐（`step 1280/7380 @ 80.2s`），预计结束时间窗已前移到 `2026-04-11 23:20-23:45 CST`。canonical 现已预登记 unattended matrix queue `outputs/queue/factor_ladder_unattended_20260411`，用于在当前 formal `E2` 结束后无条件串行接管后续 `E1 -> E2 teacher30 -> export -> E3`。 | 等 `E2 short10` 跑完并按 factor-ladder 判读规则出首个 formal gate 结论；在结果出来前，不并行再开 `E3`。unattended matrix queue 预登记的 downstream queued runs 已写入 canonical manifest：`teacher_v2_center_pool_longctx4096_short10_s42_6000_20260411_r1`、`teacher_v2_hierdec4096_teacher30_s42_6000_20260411_r1`、`teacher_v2_hierdec4096_distill_short10_s42_6000_20260411_r1`。如需跟踪进度，直接看当前 run log 和 GPU。 | `docs/superpowers/specs/2026-04-11-alphagenome-like-factor-ladder-design.md`、`reports/unet_vs_alphagenome_reassessment_20260411.md`、`docs/experiments/registry.md` |
