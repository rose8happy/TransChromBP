...
def main() -> None:
    ...
    def update_scope(scope: str, outputs: Any, counts: torch.Tensor, batch_weight: float) -> None:
        _, loss_metrics = compute_losses(outputs, counts, train_cfg.get("loss", {}))
    ...
    with torch.no_grad():
        for step_idx, batch in enumerate(eval_loader, start=1):
            ...
            update_scope("overall", outputs, profile_counts, batch_weight)
            ...
            if isinstance(region_sources, Sequence) and not isinstance(region_sources, (str, bytes)):
                for source in VALIDATION_REGION_KEYS:
                    ...
                    update_scope(source, source_outputs, source_counts, float(local_count))
    ...
    for scope in SCOPE_KEYS:
        ...
        print(
            ...
            f"loss_total={scope_result['loss_total']:.5f} "
            f"count_r={scope_result['count_pearson_full']:.4f} "
            f"profile_jsd={scope_result['profile_target_jsd_full_mean']:.4f}"
        )
