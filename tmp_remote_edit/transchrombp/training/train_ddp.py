...
def compute_genos_film_diagnostics(model: nn.Module) -> Dict[str, float]:
    """Collect current-batch FiLM / count injection usage statistics."""
    raw_model = unwrap_model(model)
...
def compute_losses(outputs: TransChromBPOutput, profile_counts: Tensor, loss_cfg: Dict[str, Any]) -> tuple[Tensor, Dict[str, Tensor]]:
...
def build_genos_runtime(
    model_cfg: Dict[str, Any],
    device: torch.device,
    rank: int,
) -> Optional[Any]:
    """Build a frozen GenosFeatureExtractor if genos_branch is enabled."""
    genos_cfg = model_cfg.get("genos_branch", {})
    if not genos_cfg.get("enabled", False):
        return None
...
def run_validation(
    ...
    genos_runtime: Optional[Any] = None,
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(val_loader, start=1):
            ...
            with torch.autocast(
                ...
                outputs = model(seq, genos_feat=genos_feat, genos_summary=genos_summary)
                _, metrics = compute_losses(outputs, profile_counts, loss_cfg)
            ...
            if isinstance(region_sources, Sequence) and not isinstance(region_sources, (str, bytes)):
                for source in VALIDATION_REGION_KEYS:
                    ...
                    _, source_loss_metrics = compute_losses(source_outputs, source_counts, loss_cfg)
...
def main() -> None:
    ...
    try:
        ...
        # Build frozen Genos extractor (outside DDP / optimizer).
        ...
        if uses_cached_genos and genos_runtime is not None:
            raise RuntimeError(
                "Both genos_cached.enabled and genos_branch.enabled are set. "
                "Cached mode must not fall back to online extraction. "
                "Disable genos_branch.enabled when using cached summaries."
            )
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            for step_idx, batch in enumerate(train_loader, start=1):
                ...
                if (step_idx % log_every == 0) and is_main_process(dist_env.rank):
                    ...
                    runtime_suffix = (
                        ...
                        f"t_genos={avg_runtime['step_time_genos_fetch']:.4f}s "
            ...
            if is_main_process(dist_env.rank):
                ...
                runtime_suffix = (
                    ...
                    f"t_genos={train_epoch_metrics['step_time_genos_fetch']:.4f}s "
