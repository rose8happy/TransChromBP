...
风格的偏差因子化统一到同一 bias-safe framework 中，用于碱基分辨率
ATAC-seq profile/count 预测。
...
本文还把 Genos、Caduceus-PS 与 NT v2 等 foundation-model 接入路线作为受控
side quest 进行验证；无论冻结 summary 注入、在线 token 融合，还是两轮更保守的
NT v2 residual head gate（先是 coarse-summary residual，后是 bins16 center-aligned
residual），都未观察到 clean gain。后者在 matched short10 对照下仍明显落后，
说明当前已评估的 foundation integration recipes 与碱基分辨率 ATAC 任务之间存在
粒度与互补性的双重错配，因此 foundation 线在本文中仅作为附录 / future work
中的边界负结果。
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{foundation-model 接入的受控负结果}。对 Genos、Caduceus-PS 和
  NT v2 的 summary / token-fusion / residual-head 路线进行验证，均未得到
  clean gain；这些结果更适合作为 appendix / future work 中的边界说明，而不是
  论文主线证据。
\end{enumerate}
...
%% -------------------------------------------------------------------
\subsection{基因组 foundation-model 接入实验}
\label{sec:genos}
%% -------------------------------------------------------------------

预训练基因组基础模型在序列级别任务上展现了显著的迁移能力，本文因此把它们作为
受控 side quest 而非主线，探索其表征是否能为 TransChromBP 提供低干扰、
可复现的互补信号。当前结果可以分为三层：Genos-1.2B 代表冻结 summary 注入，
Caduceus-PS 代表在线 token-level 融合，NT v2 residual head 代表更保守的
低扰动 residual 接法。下面先给出 Genos 的主结果，再总结后续两条 gate 的收口结论。
...
\subsubsection{后续受控 gate：Caduceus 与 NT v2}

在 Genos 之后，本文又用两条更保守的 recipe 验证“是否存在低干扰互补信号”。
第一条是 Caduceus-PS 在线 token 融合。它在工程上可以稳定训练，说明
token-level 融合并非天然不可用；但 matched tutorial A/B 只给出
near-null / marginal positive，未达到值得继续扩线的门槛。第二条是
NT v2 cached residual short10。它先以 coarse-summary residual 作为第一道 gate：
我们将 foundation 特征限制为 cached early-layer summary，并通过零初始化
residual head 只学习 debiased residual，以尽量避免直接扰动主 backbone。
该设计在 6000 双卡上已完整跑通 cache、训练、validation 和 held-out gate，
但最终 \texttt{test-full} 结果为 peak JSD=$0.3560$、peak count $r$=$0.7729$，
明显劣于当前 paper-facing 安全线（停表阈值分别为 $0.3197$ 和 $0.8400$），
因此直接触发 fail-or-unsafe。随后又按受控重入，把 residual head 改成
center-aligned 的 bins16 token-aware 版本，再做 matched short10 对照；但 full held-out
仍只有 overall $0.4625/0.7727$、peak $0.3588/0.7516$，相对 matched no-foundation
control 的 overall $0.4395/0.8457$、peak $0.3193/0.8298$ 继续落后。

这两条后续 gate 与 Genos 负结果一起，把 foundation line 的当前定位固定下来：
本文已经否定的是“当前 coarse summary 以及 bins16 center-aligned residual short10 family
没有带来 clean gain”，而不能得出“所有 token/base-level foundation 路线都无效”。
因此 foundation-model 结果在本文中应被收口为 appendix / future work 中的受控负结果，
而不是主表主线。
...
%% -------------------------------------------------------------------
\subsection{基础模型、互补性与任务粒度}
%% -------------------------------------------------------------------

Genos、Caduceus-PS 和 NT v2 三条受控 gate 共同给出了一个更稳的经验边界：
当前 foundation model 并不是完全没有信号，但它缺少的是能够在 base-resolution
ATAC 任务上以低干扰、可复现方式转化为净增益的互补性。Genos 说明冻结的全局
summary 会丢掉碱基分辨率所需的空间结构；Caduceus-PS 说明 token-level 在线融合
在工程上可以安全，但在当前 recipe 下增益接近于零；NT v2 residual head 则进一步
说明，即便把注入限制到“零初始化、只预测 debiased residual”的保守低扰动路径，
从 coarse-summary residual 到 bins16 center-aligned residual 的两轮 gate 也都没能把
held-out 结果拉回 matched baseline。

这组结果最重要的含义不是“foundation model 已被彻底证伪”，而是：
当前 recipe 只支持把 foundation 线写成受控 side quest，而不支持把它写成新的主线。
尤其需要强调的是，当前 NT v2 residual head 虽然已经从“粗 summary /
低带宽 residual”推进到“center-aligned bins16 residual”，但它仍属于同一条
low-disturbance residual short10 family，而不是更一般的 token/base-level
foundation 路线。因此本文对 foundation 的结论必须保持克制：否定的是当前已测试
family，没有否定全部更细粒度路线。

对于序列级别的分类任务（如 OCR 检测），全局均值池化的 embedding 已经足够有效，
这也被本文的 OCR 基准复现所验证。但对于碱基分辨率的回归任务（如 ATAC profile
预测），同样的 embedding 会天然丢失任务所需的空间信息。再加上当前任务专用
baseline 已经很强，外部弱信号越过噪声门槛的难度更高。这构成了一个“互补性悖论”：
越是强 baseline，越需要外部信号提供真正新的、与主干不重叠的信息；而 coarse
summary 往往最先丢掉的，正是这类对 base-resolution 任务最关键的结构信息。
...
结果支撑的最终默认模型。作为受控 side quest 的 foundation-model 接入实验则
进一步表明，无论冻结 summary 注入、在线 token 融合还是当前低带宽 residual
方案，都尚未给出 clean gain；这些结果更适合作为 appendix / future work 中的
边界说明，而不是改写本文主线结论。
