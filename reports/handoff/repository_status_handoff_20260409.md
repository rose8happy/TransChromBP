...
这条 snapshot branch 已固定在 clean 提交 `4ffc6e7 (wip: snapshot dual track pivot)`，是近期 dual-track pivot 的历史代码入口。当前 master-side 的 canonical 说明见 `reports/handoff/dual_track_pivot_snapshot_20260409.md`，核心内容包括：
...
- `TRACKING.md` 已把规则源指向 `docs/plan/dual_machine_experiment_charter_20260409.md`，并把它作为 live 入口，而不是让 handoff 自己承担优先级裁决
- 旧的 `reports/handoff/session_handoff_multiscale_and_next_tasks_20260409.md` 仍保留着 `msdec_v1_s2 / skipprobe_wide` 的历史快照
- 真实现场已经切到：
  - `6000` 的 A6000 formal gate 已收口并判 `fail`
  - `6000` 的 AlphaGenome v2 sidecar 已收口并判 `pass`
  - `6002` 的 `U-Net-lite v1` 已收口并判 `no-go / stop`
- `dual-track-20260409` 的 pivot 计划与切换记录现已在 `reports/handoff/dual_track_pivot_snapshot_20260409.md` 里给出 canonical 恢复入口；不再要求依赖 live mounted worktree
...
1. `docs/plan/dual_machine_experiment_charter_20260409.md`：回答“当前什么实验合法、什么优先、什么时候才允许切回论文”
2. `TRACKING.md`：回答“当前 live 进度和下一步是什么”
3. 本文件：回答“现在真实在发生什么”
4. `reports/governance/project_plan_code_review_20260405.md`：回答“最近代码为什么会改成这样”
5. `reports/handoff/session_handoff_multiscale_and_next_tasks_20260409.md`：保留为旧双机排程快照
6. `reports/handoff/dual_track_pivot_snapshot_20260409.md`
