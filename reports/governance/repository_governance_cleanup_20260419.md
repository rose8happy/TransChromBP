...
5. 把几个不应长期留在仓库根目录的单文件归位：
   - `TRAINING_ANALYSIS.md` → `reports/analysis/chrombpnet_tutorial_bias_training_analysis_20260123.md`
   - `REPRODUCTION_NOTES.md` → `docs/archive/ops/chrombpnet_reproduction_notes.md`
...
1. 从临时保护分支 `master-wip-20260419-pre-governance-merge` 只挑回真正仍有价值的内容，而不是整支回滚：
   ...
   - `reports/external/chatgpt_bundle_loss_balance_20260416/` 外发专题包
   - `reports/governance/repository_hygiene_cleanup_20260417.md`
...
4. 完成一轮 mounted worktree 审计并写入 `reports/governance/worktree_registry_audit_20260419.md`：
