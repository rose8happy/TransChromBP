...
class TransChromBP(nn.Module):
    ...
    def __init__(
        ...
        # ── Optional Caduceus token fusion adapter ──
        self.caduceus_adapter: Optional[CaduceusTokenAdapter] = None
        ...
        # ── Optional generic foundation adapters ──
        self.foundation_cross_adapter: Optional[FoundationCrossAttentionAdapter] = None
        self.foundation_residual_head: Optional[FoundationResidualHead] = None
    ...
    def forward(
        ...
        caduceus_feat: Optional[Tensor] = None,
        genos_summary: Optional[Tensor] = None,
        foundation_tokens: Optional[Tensor] = None,
        foundation_summary: Optional[Tensor] = None,
    ) -> TransChromBPOutput:
        """Forward pass.
        ...
        Args:
            ...
            caduceus_feat: optional Caduceus hidden-state features [B, L, caduceus_dim]
            genos_summary: optional cached Genos global summary [B, genos_dim]
            foundation_tokens: optional external foundation tokens [B, M, H]
            foundation_summary: optional external foundation summary [B, H]
        """
        ...
        if caduceus_feat is not None and self.caduceus_adapter is not None:
            tokens = self.caduceus_adapter(tokens, caduceus_feat)
        if genos_feat is not None and self.genos_adapter is not None:
            tokens = self.genos_adapter(tokens, genos_feat)
        if foundation_tokens is not None and self.foundation_cross_adapter is not None:
            tokens = self.foundation_cross_adapter(tokens, foundation_tokens)
        ...
        if self.foundation_residual_head is not None and (
            foundation_summary is not None or foundation_tokens is not None
        ):
            profile_delta, count_delta = self.foundation_residual_head(
                encoded,
                foundation_summary=foundation_summary,
                foundation_tokens=foundation_tokens,
            )
            profile_signal = profile_signal + profile_delta
            count_signal = count_signal + count_delta
...
def build_transchrombp_from_config(config: Dict[str, Any]) -> TransChromBP:
    ...
    # ── Attach optional Caduceus token adapter (online mode) ──
    if caduceus_cfg.get("enabled", False):
        d_model = int(seq_cfg.get("d_model", 256))
        fusion_mode = str(caduceus_cfg.get("fusion_mode", "gated_residual")).lower()

        if fusion_mode not in {"gate", "gated_residual"}:
            raise ValueError(
                "Unsupported caduceus_branch.fusion_mode="
                f"{fusion_mode!r}; expected 'gate' or 'gated_residual'"
            )
        model.caduceus_adapter = CaduceusTokenAdapter(
            d_model=d_model,
            caduceus_hidden_size=int(caduceus_cfg.get("hidden_size", d_model)),
            gate_bias_init=float(caduceus_cfg.get("gate_bias_init", -2.0)),
        )
    ...
    if foundation_cfg.get("enabled", False):
        d_model = int(seq_cfg.get("d_model", 256))
        mode = str(foundation_cfg.get("mode", "distill_only")).strip().lower()
        foundation_hidden_size = int(
            foundation_cfg.get("hidden_size", foundation_cfg.get("feature_hidden_size", d_model))
        )
        if mode == "cross_attention":
            model.foundation_cross_adapter = FoundationCrossAttentionAdapter(
                d_model=d_model,
                foundation_hidden_size=foundation_hidden_size,
                n_heads=int(foundation_cfg.get("cross_attn_heads", 4)),
                dropout=float(seq_cfg.get("dropout", 0.1)),
                gate_bias_init=float(foundation_cfg.get("gate_bias_init", -4.0)),
            )
        elif mode == "residual_head":
            model.foundation_residual_head = FoundationResidualHead(
                d_model=d_model,
                foundation_hidden_size=foundation_hidden_size,
            ...
            raise ValueError(
                ...
                f"{mode!r}; expected one of 'cross_attention', 'residual_head', 'distill_only'"
            )
