...
> 历史执行稿。当前仓库的同步入口已切到 `./scripts/sync_project.sh publish-runtime-6000` / `publish-runtime-6002` / `pull-results-*`；文中旧的 `deploy` 与独立 staging deploy helper 仅保留为历史上下文，不再作为现行 SOP。
...
4. `TransChromBP corrected-B` 的 L2 配置与 selector 已就位
   - 文件：
     ...
     - 历史上曾有独立 staging deploy helper；现已退役，改由 `publish-runtime-6000` 承担统一发布
   - 作用：
...
这一步在当时只同步少量 matched-budget 配置和 selector。当前独立 staging helper 已退役；若今天要重跑这条历史流程，应改为通过 `publish-runtime-6000` 统一发布 canonical repo，再在 6000 侧挑选实际需要的配置与脚本。
...
| Step | 动作 | 产出 | Layer |
|---:|---|---|---|
| 1 | `./scripts/sync_project.sh publish-runtime-6000` | 6000 代码同步 | — |
| 2 | 跑 `tutorial official fidelity` | 官方锚点指标 | L1 |
| 3 | 跑 `tutorial official controlled` (L2) | 官方 L2 逐 epoch checkpoint | L2 |
| 4 | `publish-runtime-6000` 后在 6000 侧选取所需配置 | TransChromBP 配置同步 | — |
